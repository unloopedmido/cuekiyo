...
export const api = {
  ...
  loadThemes: (id: string) =>
    request<{ jobId: string }>(`/projects/${id}/load-themes`, { method: "POST" }),
  ...
  confirmRenderOrder: (id: string) =>
    request<{ jobId: string }>(`/projects/${id}/render-order/confirm`, { method: "POST" }),
  updateRenderOrder: (id: string, songIds: string[]) =>
    request<{ ok: boolean }>(`/projects/${id}/render-order`, {
      method: "PUT",
      body: JSON.stringify({ song_ids: songIds }),
    }),
  retry: (id: string) => request<{ jobId?: string }>(`/projects/${id}/retry`, { method: "POST" }),
  cancel: (id: string) => request<{ ok: boolean }>(`/projects/${id}/cancel`, { method: "POST" }),
  listJobs: (id: string) => request<Job[]>(`/projects/${id}/jobs`),
  projectLogs: (id: string) =>
    request<{ id: string; message: string; level: string; created_at: string }[]>(
      `/projects/${id}/logs`,
    ),
  ...
  startStage: (id: string) =>
    request<{ jobId: string }>(`/projects/${id}/stage/start`, { method: "POST" }),
  openOutputFolder: (id: string) =>
    request<{ folder: string }>(`/projects/${id}/output/open-folder`, { method: "POST" }),
};
